...
# shorts_auto(C:\shorts_auto\scripts\instagram_upload.py)와 동일한 패턴이지만, 토큰 파일은
# 사주랩 인스타 계정 전용으로 완전히 분리되어 있다 — daily.factlab 계정과 절대 안 겹친다.
...
def get_token():
    if not os.path.exists(TOKEN_PATH):
        raise SystemExit(
            ...
            "--exchange-code CODE 로 최초 인증을 완료해주세요 (사주랩 인스타 계정으로 로그인해서 승인)."
        )
...
def upload_carousel(image_urls, caption):
    """카드뉴스(캐러셀) 게시. 이미지마다 개별 컨테이너(is_carousel_item)를 만든 뒤,
    그 id들을 묶은 부모 컨테이너(media_type=CAROUSEL)를 발행한다."""
    token_data = get_token()
    access_token = token_data["access_token"]
    user_id = token_data["user_id"]
    ...
    for i, image_url in enumerate(image_urls):
        ...
        child_ids.append(item_resp.json()["id"])
        print(f"슬라이드 {i + 1}/{len(image_urls)} 컨테이너 생성됨: {child_ids[-1]}")
    ...
    create_resp.raise_for_status()
    container_id = create_resp.json()["id"]
    print(f"캐러셀 컨테이너 생성됨: {container_id}, 처리 대기 중...")
    ...
    for _ in range(30):
        ...
        if status == "FINISHED":
            break
        if status == "ERROR":
            raise RuntimeError("인스타그램 캐러셀 처리 실패")
        time.sleep(3)
    else:
        raise RuntimeError("캐러셀 처리 시간 초과")
