...
| 🟢 Low → **패치 완료** | 후기 등록 API가 paymentId 소유 여부를 쿠키로 재확인하지 않음(IDOR 유사 패턴) | ✅ 수정·테스트·배포 완료 |
...
### 2-1. POST `/api/reviews` — 🟢 Low, **✅ 패치 완료**
**(추가 업데이트, 같은 날 수정됨)** 기존 "결제 완료(PAID) 주문인지" 검사 바로 뒤에 `saju_order_{paymentId}` 쿠키 검증을 추가했습니다(`src/app/api/reviews/route.ts`). 기존 검사(주문 없음/미결제 → 403 "결제 완료된 주문에서만...", 중복 후기 → 409)는 메시지 하나 안 바꾸고 그대로 뒀고, UI(`ReviewForm.tsx`)도 전혀 변경하지 않았습니다 — 같은 브라우저의 same-origin fetch는 쿠키가 자동으로 실리기 때문입니다. 실제 API로 테스트 완료: 쿠키 없음/위조 쿠키 → 403(신규), 정상 쿠키 → 201 성공(기존 유지), 같은 주문 재등록 → 409(기존 유지), 미결제 주문은 쿠키 위조해도 → 403 "결제 완료된 주문에서만"(기존 유지). 아래는 수정 전 발견 당시의 원문입니다.
